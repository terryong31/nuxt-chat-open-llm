...
const { thinkingBudget, temperature, topP, maxTokens } = useLLMSettings()
...
const { messages, status, error, sendMessage, regenerate, stop } = useChat({
  ...
  onData: async (dataPart) => {
    ...
    } else if (dataPart.type === 'data-metrics') {
      const lastMsg = messages.value[messages.value.length - 1]
      if (lastMsg && lastMsg.role === 'assistant') {
        if (!lastMsg.parts) lastMsg.parts = []
        const existingIdx = lastMsg.parts.findIndex(p => (p as { type?: string }).type === 'metrics' || (p as { type?: string }).type === 'data-metrics')
        const metricsData = (dataPart as Record<string, unknown>).data
        if (existingIdx >= 0) {
          lastMsg.parts[existingIdx] = { type: 'metrics', data: metricsData } as unknown as (typeof lastMsg.parts)[0]
        } else {
          lastMsg.parts.push({ type: 'metrics', data: metricsData } as unknown as (typeof lastMsg.parts)[0])
        }
      }
    }
  },
